...
public class TotemConsumeEvent implements Listener {
    ...
    public void totemNerf(EntityResurrectEvent event){
        ...
        if (((Player) event.getEntity()).getInventory().getItemInMainHand().getType() == Material.TOTEM_OF_UNDYING || ((Player) event.getEntity()).getInventory().getItemInOffHand().getType() == Material.TOTEM_OF_UNDYING) {
            ...
            if (!instance.getConfig().getBoolean("TotemFail.Enable")) return;
            ...
            if (Main.getInstance().getConfig().contains("TotemFail.FailProbs." + Main.getInstance().getDays())) {
                failProb = Objects.requireNonNull(Objects.requireNonNull(instance.getConfig().getInt("TotemFail.FailProbs." + Main.getInstance().getDays())));
                containsDay = true;
            } else {
            ...
            if (Main.getInstance().getDays() >= 40) {
                if (Main.getInstance().getDays() < 60) {
                    ...
                    totemMessage = Objects.requireNonNull(instance.getConfig().getString("TotemFail.PlayerUsedTotemsMessage").replace("{ammount}", "tres").replace("%player%", player));
                }
            }
            ...
            if (failProb == 100) {
                ...
                if (instance.getDays() < 40) {
                    ...
                    int neededTotems = (instance.getDays() < 60 ? 2 : 3);
                    int totems = p.getInventory().all(Material.TOTEM_OF_UNDYING).size();

                    if (p.getInventory().getItemInOffHand() != null && p.getInventory().getItemInOffHand().getType() == Material.TOTEM_OF_UNDYING) totems++;
