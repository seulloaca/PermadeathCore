...
public class GatoGalacticoTask extends BukkitRunnable {
    ...
    public void run() {
        ...
        if (time > 0) {
            ...
            Bukkit.broadcastMessage(main.format("&eUn gato galáctico invocará un mob al azar en: &b" + time));
        ...
        if (time == 0) {
            ...
            if (type == EntityType.CAT) {
                ...
                instance.getNmsAccesor().setMaxHealth(jess, 500.0D, true);

                instance.getNmsAccesor().setMaxHealth(carlos, 150.0D, true);

                instance.getNmsAccesor().setMaxHealth(ravager, 240.0D, true);
                ...
                ravager.getPersistentDataContainer().set(new NamespacedKey(instance, "ultra_ravager"), PersistentDataType.BYTE, (byte) 1);
                ...
                main.getNmsAccesor().setMaxHealth(demon, 1350.0D, true);

            } else if (type == EntityType.SKELETON) {

                entidad = "Esqueleto de Clase";
                instance.getNmsHandler().spawnNMSEntity("Skeleton", EntityType.SKELETON, cat, CreatureSpawnEvent.SpawnReason.NATURAL);
                ...
                instance.getNmsHandler().spawnNMSEntity("Phantom", EntityType.PHANTOM, cat, CreatureSpawnEvent.SpawnReason.NATURAL);
                ...
                if (r == 2) {
                    ...
                    main.getNmsAccesor().setMaxHealth(GhastDemon, HPGenerator, true);
            ...
            if (type == EntityType.GIANT) {
                ...
                entidad = "Gigante";
                instance.getNmsHandler().spawnNMSCustomEntity("CustomGiant", EntityType.GIANT, cat, CreatureSpawnEvent.SpawnReason.CUSTOM);
            }
            ...
            if (type == EntityType.WITHER_SKELETON) {
                ...
                WitherSkeleton skeleton = (WitherSkeleton) instance.getNmsHandler().spawnNMSEntity("SkeletonWither", EntityType.WITHER_SKELETON, cat, CreatureSpawnEvent.SpawnReason.CUSTOM);
                EntityEquipment eq = skeleton.getEquipment();

                instance.getNmsAccesor().setMaxHealth(skeleton, 80.0D, true);

                skeleton.setCustomName(instance.format("&6Wither Skeleton Emperador"));
            ...
            Bukkit.broadcastMessage(main.format("&eUn gato galáctico ha invicado un(a) &c&l" + entidad + " &7(" + cat.getX() + ", " + cat.getY() + ", " + cat.getZ()));
            this.cancel();
        }
    }
}
